...
1. ~~时间预算未对齐~~ **已解决（2026-09-09）**：用户确认每周 5 次 × 3 小时 = 15 小时，满配 12 h 20 m 有充足余量，走满配。排期表见第 4 节。
2. ~~数据许可待判断~~ **已解决（2026-09-09）**：用户确认本项目**不用于任何商业活动，只是个人练习**，`cc-by-nc-2.0` 的非商用条款不触及边界。此判断由用户作出并记录在此；若日后用途变化（并入公司产品、对外发布、商业化），需要重新评估。

...
1. ~~每周可投入的小时数~~ **已确认（2026-09-09）**：每周 5 次 × 3 小时 = 15 小时。M03 满配 12 h 20 m，**走满配**，一次一张卡，每次留 60 分钟给内网首次运行的摩擦。六次做完（本周 Day 0–4，下周第一次 Day 5 + Gate）。
2. **数据已拷到 V100**，且 `python lab/scripts/check_data_layout.py` 通过。
3. ~~许可边界~~ **已确认（2026-09-09）**：用户确认本项目不用于任何商业活动，只是个人练习，`cc-by-nc-2.0` 的非商用条款不触及边界。用途若变化需重新评估。
